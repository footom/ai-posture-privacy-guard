...
├── docker-compose.yml    # 預設compose檔（僅dashboard，適用於Windows / macOS / Linux）
├── docker-compose.linux.yml # Linux專用疊加檔（額外加入monitor服務）
├── .gitignore
├── .dockerignore
│
├── src/                  # 核心程式碼
...
僅啟動 `dashboard`（Streamlit 儀表板，對外開放 `8501` 埠）。`monitor`（主監控程式）依賴攝影機直通與 Windows 原生鎖屏 API（`LockWorkStation()`），無法在容器內運作，請直接在本機執行：
...
會在`dashboard`之外額外疊加啟動`monitor`（掛載`/dev/video0`攝影機）。注意鎖屏功能在Linux容器內仍不可用，觸發資安風險時會改為跳出 GUI 強制警告視窗，而非實際鎖屏。

---

## 測試

[![Tests](https://github.com/footom/ai-posture-privacy-guard/actions/workflows/tests.yml/badge.svg?branch=main)](https://github.com/footom/ai-posture-privacy-guard/actions/workflows/tests.yml/badge.svg?branch=main)

本專案針對核心商業邏輯撰寫了單元測試，共**55個測試案例**，涵蓋：
...
- 跨平台支援：自動螢幕鎖定功能（`LockWorkStation`）依賴Windows API，僅在Windows環境下執行實體鎖屏；於Linux / macOS / 容器環境下，若觸發資安風險則不會實際鎖屏，而是自動切換為GUI強制警告視窗並寫入事件紀錄。
...
- Docker 部署已依主機作業系統拆分為兩個 compose 檔（`docker-compose.yml` 為預設，僅含 dashboard；`docker-compose.linux.yml` 為Linux專用疊加檔，額外加入 monitor），避免在Windows / macOS上誤用僅支援Linux攝影機裝置節點（`/dev/video0`）的設定。
